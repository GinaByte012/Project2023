...
app.get('/home', function (request, response) {
  ...
  client.query(`SELECT * FROM user`, function (error, data) {
    console.log(data)
    var html =
      `
    <!doctype html>
      <html>
      <head>
        <title>nodejs - mariadb </title>
        <meta charset = "utf-8">
      </head>
      <body>
        <p>${data[0].user_num} / ${data[0].name} / ${data[0].id} / ${data[0].passwd} </p> 
        <p>${data[1].user_num} / ${data[1].name} / ${data[1].id} / ${data[1].passwd} </p> 
        <p>${data[2].user_num} / ${data[2].name} / ${data[2].id} / ${data[2].passwd} </p> 
      </body>
      </html>
    `
...
// 'register'
app.get('/src/userManagements/signup', function (request, response) {
  auth.register(request, response);
});
app.post('/src/userManagements/signup_process', function (request, response) {
...
app.listen(60001, () => console.log('Example app listening on port 60001'));    // 웹서버 작동, 웹의 요청을 수신

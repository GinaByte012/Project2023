...
module.exports = {
  ...
  // // GET 'User Detail Update'  (사용자 개인정보 수정 화면)
  // userUpdatePage: function (req, res) {
  //   console.log('[GET] User Information Update for Admin   in adminControl.js');
  //   var userNum = req.params.user_num;
  ...
  // // PATCH 'User Update'    // (사용자 개인정보 수정 요청)
  // userUpdate: function (req, res) {
  //   console.log('[PATCH] UserDetail for Admin   in beginning.js');
  //   const userNum = req.params.user_num;
  //   const inputName = req.body.name;
  //   const inputNickname = req.body.nickname;

  //   // 관리자가 수정한 정보 DB에 저장
  //   db.query(`UPDATE user SET name=?, nickname=? WHERE user_num=?`,
  //     [inputName, inputNickname, userNum], function (error, result) {
  //       if (error) {
  //         console.log('DB 오류: '.error);
  //         res.status(500).json('Fail: GET USER LIST  ::  DB');
  //       }
  ...
  //       else {
  //         console.log('SUCCESS: Update User Information Successfully!!');
  //         res.status(200).json('SUCCESS: Update User Information');
  //       }
  //     })
  // },
  ...
  qnaReply: function (req, res) {
    ...
    // db.query(`UPDATE qna SET replied=?, reply=?, admin_num=? WHERE qna_num=?`,
    //   [newReplied, inputReply, adminNum, qnaNum], function (error, result) {
    //     if (error) {
    //       console.log('DB 오류:', error);
    ...
    db.query(`SELECT * FROM manual WHERE manual_num = ?`,
      [manualNum], function (error, result) {
        ...
        // DB에서 정상적으로 정보 받아옴
          ...
          // const manuals = result[0].map((row) => {
          //   return {
          //     id: row.manual_num,
          //     types: row.types,
          //     title: row.title,
          //     content: row.content,
          //     eligible_item: row.eligible_item,
          //     ineligible_item: row.ineligible_item
          //   };
          // });
          console.log(manual);
  ...
  GROUP BY town_num`, function (error, result) {
    ...
    //   db.query(`SELECT * FROM trashcan`, function (error, result) {
    //   if (error) {
    //     console.log("Error in DB Process  ::  Cannot SELECT from DB");
    //     res.status(500).json('Fail: DB ERROR. ');
    //   }
  ...
  trashcanUpdate: function (req, res) {
    ...
    // 받아온 정보 DB에 UPDATE
    db.query(`UPDATE trashcan SET town=?, street=?, detail=?, placed=?, types=? WHERE trashcan_num=?`,
      [town, street, detail, placed, types, trashcanNum], function (error, result) {
        if (error) {
          console.log('DB 오류: '.error);
          res.status(500).json('Fail: UPDATE Trashcan Locatiobn Information  ::  Admin - DB');
        }
        else {
          console.log("Success UPDATE a Trashcan Locatiobn !!");
          res.status(200).json('Success: Trashcan Update :: Update DB information SUCCESSFULLY!!!');
        }
      }
    )
  },
  ...
  trashcanDelete: function (req, res) {
    ...
    db.query('DELETE FROM trashcan WHERE trashcan_num=?', [trashcanNum], function (error, result) {
      if (error) {
        console.log('DB 오류: '.error);
        res.status(500).json('Fail: DELETE Trashcan Location  ::  Admin - DB');
      }
      else {
        console.log("Success DELETE a Trashcan Location!!");
        res.status(200).json('Success: DELETE Trashcan Location  ::  DELETE DB information SUCCESSFULLY!!!');
      }
    });
  },
  ...
  feedbackPage: function (req, res) {
    ...
    db.query(`SELECT * FROM classification`, function (error, result) {
      ...
      // AI 분류 결과 성공적으로 불러옴
        ...
        console.log("\n>>> console.start ==================\n");    // 매뉴얼 정보 전달
        console.log(classification);    // 매뉴얼 정보 전달
        console.log("\n ================================================");    // 매뉴얼 정보 전달
        console.log(" ============================ console.finish <<<\n");    // 매뉴얼 정보 전달
        res.send(classification);    // 매뉴얼 정보 전달
  ...
  adminDetailUpdate: function (req, res) {
    ...
    // DB Update
    db.query(`UPDATE admin SET name=?, department=?, isUsed=?, isAuth=? WHERE admin_num=?`,
      [name, department, isUsed, isAuth, adminNum], function (error, result) {
        if (error) {
  ...
  adminDetailDelete: function (req, res) {
    ...
    db.query(`UPDATE admin SET isUsed='N' WHERE admin_num=?`, [adminNum], function (error, result) {
      if (error) {
